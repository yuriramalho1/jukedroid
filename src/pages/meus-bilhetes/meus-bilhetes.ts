...
@IonicPage()
@Component({
  selector: 'page-meus-bilhetes',
  templateUrl: 'meus-bilhetes.html',
})
export class MeusBilhetesPage {
  bilhete : Bilhete
  bilhetes : Bilhete[] = [];
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public bs : BilheteService, private us : UsuarioService,
              public printer: PrinterProvider, public storage : Storage, public loadCtrl: LoadingController, public modalCtrl: ModalController,
              public alertCtrl: AlertController, private plt: Platform) {
  ...
  buscarTodosBilhetes(dataInicio, dataFinal : string){
    ...
    this.us.getUsuario().then(usuario => {
      this.usuario = usuario;
      this.bs.getBilhetesByUsuario(usuario, dataInicio, dataFinal).then(lista => {
        this.bilhetes = lista
